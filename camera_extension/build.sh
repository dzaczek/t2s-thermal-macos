...
#   ./build.sh --prepare       prepare version and signing settings for Xcode
...
case "$MODE" in
    ""|--run|--prepare|--release) ;;
    *) echo "Usage: $0 [--run|--prepare|--release]" >&2; exit 1 ;;
esac
...
# Stay ahead of the installed app even when this checkout's counter is old.
# The generated xcconfig also keeps direct Xcode builds on the same number.
BUILD_FILE="build_number.txt"
BUILD_NUMBER=$(awk '/^CURRENT_PROJECT_VERSION = / { print $3 }' Version.xcconfig)
if ! [[ "$BUILD_NUMBER" =~ ^[0-9]+$ ]]; then
    echo "Version.xcconfig must contain an integer CURRENT_PROJECT_VERSION." >&2
    exit 1
fi
for PREVIOUS in \
    "$(cat "$BUILD_FILE" 2>/dev/null || true)" \
    "$(awk '/^CURRENT_PROJECT_VERSION = / { print $3 }' LocalBuild.xcconfig 2>/dev/null || true)" \
    "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$APP/Contents/Info.plist" 2>/dev/null || true)"; do
    if [[ "$PREVIOUS" =~ ^[0-9]+$ ]] && (( 10#$PREVIOUS >= BUILD_NUMBER )); then
        BUILD_NUMBER=$((10#$PREVIOUS + 1))
    fi
done
echo "$BUILD_NUMBER" > "$BUILD_FILE"
cat > LocalBuild.xcconfig <<CONFIG
// Generated by build.sh. Local to this Mac; do not commit.
...
if [ "$MODE" = "--prepare" ]; then
    echo "==> Ready for Xcode (build $BUILD_NUMBER): $PWD/T2SCamera.xcodeproj"
    exit 0
fi
